...
function auth_forgot_password($body) {
    ...
    // Sprawdź wymagane stałe konfiguracyjne
    ...
    try {
        ...
        if (!$sent) {
            http_response_code(500);
            echo json_encode(['message' => 'Nie udało się wysłać e-maila. Sprawdź konfigurację MAIL_FROM w config.php — musi to być skrzynka istniejąca na OVH.']);
            return;
        }
...
function auth_reset_password($body) {
    ...
    try {
        ...
        // Najpierw waliduj token — przed sprawdzeniem hasła
        $stmt = $pdo->prepare(

...
function handle_plans($method, $id, $body) {
    ...
    $pdo  = get_pdo();
    $stmt = $pdo->query('SELECT id, name, max_tires AS maxTires, has_customers AS hasCustomers, has_actions AS hasActions, price_monthly AS priceMonthly FROM plans WHERE is_active = 1 ORDER BY sort_order');
    $rows = $stmt->fetchAll();
    foreach ($rows as &$r) {
